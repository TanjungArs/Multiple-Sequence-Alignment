...
def pipeline(skip_config, res_dir, clean_run,msa_filename,tree_filename,pipeline_path='/bioseq/spartaabc/pipeline/',
			 ...
			 b_num_top=100, num_alignments=200, submodel_params="amino", num_simulations=100000, num_burnin=10000, minRL=80, maxRL=500):
	...
	if skip_config["sparta"]:
		runs.create_sims_from_data(data_name='', ow_flag=False,
							...
							num_burnin=num_burnin, minRL=minRL, maxRL=maxRL) # Running spartaABC C++ code to get simulations (through another python script)
		if clean_run:
